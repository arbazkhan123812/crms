...
@section('content')
        ...
        <div class="page-header mb-4">
            <div class="row align-items-center">
                ...
                <div class="col-lg-4 text-lg-right mt-3 mt-lg-0">
                    <a href="{{ route('admin.leads.index') }}" class="btn btn-priamry mr-2"><i
        ...
        <div class="card detail-tabs-card">
            <div class="card-body">
                <ul class="nav nav-pills lead-detail-tabs mb-4">
                    @if($canViewNotes)
                        <li class="nav-item"><a class="nav-link {{ $defaultTab === 'notes-pane' ? 'active' : '' }}" data-toggle="pill" href="#notes-pane">Notes</a></li>
                    @endif
                    @if($canViewActivities)
                        <li class="nav-item"><a class="nav-link {{ $defaultTab === 'activities-pane' ? 'active' : '' }}" data-toggle="pill" href="#activities-pane">Activities</a></li>
                    @endif
                    @if($canViewTasks)
                        <li class="nav-item"><a class="nav-link {{ $defaultTab === 'tasks-pane' ? 'active' : '' }}" data-toggle="pill" href="#tasks-pane">Tasks</a></li>
                    @endif
                    @if($canViewCalls)
                        <li class="nav-item"><a class="nav-link {{ $defaultTab === 'calls-pane' ? 'active' : '' }}" data-toggle="pill" href="#calls-pane">Calls</a></li>
                    @endif
                    @if($canViewMeetings)
                        <li class="nav-item"><a class="nav-link {{ $defaultTab === 'meetings-pane' ? 'active' : '' }}" data-toggle="pill" href="#meetings-pane">Meetings</a></li>
                    @endif
                    @if($canViewEmails)
                        <li class="nav-item"><a class="nav-link {{ $defaultTab === 'emails-pane' ? 'active' : '' }}" data-toggle="pill" href="#emails-pane">Emails</a></li>
                    @endif
                </ul>
                <div class="tab-content">
                            ...
                            <div class="timeline-card editable-record" data-kind="note" data-id="{{ $note->id }}"
                                ...
                                <div class="d-flex justify-content-between align-items-start">
                                    ...
                                    <div class="text-right"><small
                                        ...
                                        @if($canManageNotes)
                                            <div class="mt-2 btn-group btn-group-sm"><button class="btn btn-light" type="button"
                                                    onclick="event.stopPropagation(); editNote({{ $note->id }}, @js($note->note))"><i
                                                        class="fas fa-edit"></i></button><button class="btn btn-light text-danger"
                                                    type="button" onclick="event.stopPropagation(); deleteNote({{ $note->id }})"><i
                    ...
                    @if($canViewActivities)
                    <div class="tab-pane fade {{ $defaultTab === 'activities-pane' ? 'show active' : '' }}" id="activities-pane">
                        <div class="row">
                            ...
                            <div class="timeline-card editable-record" data-kind="task" data-id="{{ $task->id }}"
                                ...
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        ...
                                        <div class="text-muted small mb-2">Assigned to
                                            {{ optional($task->assignedTo)->username ?? optional($task->assignedTo)->name ?? '-' }}
                                        </div>
                                        <p class="mb-1">{{ $task->description ?: 'No task description' }}</p><small
                                            class="text-muted">Due
                                    ...
                                    <div class="text-right"><span
                                        ...
                                        @if($canManageTasks)
                                            <div class="btn-group btn-group-sm"><button class="btn btn-light" type="button"
                                                    onclick="event.stopPropagation(); editTaskFromCard(this)"><i
                                                        class="fas fa-edit"></i></button>@if($task->status !== 'completed')<button
                                                            class="btn btn-light text-success" type="button"
                                                            onclick="event.stopPropagation(); markTaskCompleted({{ $task->id }})"><i
                                                        class="fas fa-check"></i></button>@endif<button
                                                    class="btn btn-light text-danger" type="button"
                                                    onclick="event.stopPropagation(); deleteTask({{ $task->id }})"><i
                                                        class="fas fa-trash-alt"></i></button></div>
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <div class="timeline-card editable-record" data-kind="call" data-mode="scheduled"
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                ...
                                                <div class="text-muted small mb-2">{{ ucfirst($call->call_type) }} call by
                                                    {{ optional($call->calledBy)->username ?? optional($call->calledBy)->name ?? '-' }}
                                                </div>
                                            ...
                                            <div class="text-right"><span
                                                ...
                                                @if($canEditCalls || $canCompleteCalls || $canDeleteCalls)
                                    ...
                                    <div class="timeline-card editable-record" data-kind="call" data-mode="logged"
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            ...
                                            <div class="text-right"><span
                                                ...
                                                @if($canEditCalls || $canDeleteCalls)
                                                    <div class="btn-group btn-group-sm">
                                                        @if($canEditCalls)
                                                            <button class="btn btn-light" type="button"
                                                                onclick="event.stopPropagation(); editCallFromCard(this)"><i
                    ...
                    @if($canViewMeetings)
                    <div class="tab-pane fade {{ $defaultTab === 'meetings-pane' ? 'show active' : '' }}" id="meetings-pane">
                        <div class="row">
                            <div class="col-lg-6 mb-4">
                                <div class="section-toolbar mb-3">
                                    <h5 class="section-title mb-0">Scheduled Meetings</h5>
                                    @if($canScheduleMeetings)
                                        <button class="btn btn-sm btn-primary" type="button" onclick="openMeetingModal()">Schedule
                                            Meeting</button>
                                    @endif
                                </div>
                                    ...
                                    <div class="timeline-card editable-record" data-kind="meeting" data-id="{{ $meeting->id }}"
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            ...
                                            <div class="text-right"><span
                                                ...
                                                @if($canEditMeetings || $canCompleteMeetings || $canDeleteUpcomingMeetings)
                                                    <div class="btn-group btn-group-sm">
                                                        @if($canEditMeetings)
                                                            <button class="btn btn-light" type="button"
                            ...
                            <div class="col-lg-6 mb-4">
                                ...
                                @if(auth()->user()->can('leads_viewpastmeeting') || $canScheduleMeetings)
                                    ...
                                    <div class="timeline-card editable-record" data-kind="meeting" data-id="{{ $meeting->id }}"
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                ...
                                                <p class="mb-1">
                                                    {{ $meeting->notes ?: ($meeting->description ?: 'No meeting notes') }}</p>
                                                <small
                                            ...
                                            <div class="text-right"><span
                                                ...
                                                @if($canEditMeetings || $canDeletePastMeetings)
                                                    <div class="btn-group btn-group-sm">
                                                        @if($canEditMeetings)
                                                            <button class="btn btn-light" type="button"
                                                                onclick="event.stopPropagation(); editMeetingFromCard(this)"><i
                    ...
                    <div class="tab-pane fade {{ $defaultTab === 'emails-pane' ? 'show active' : '' }}" id="emails-pane">
                        <div class="section-toolbar mb-3">
                            ...
                            @if($canSendEmails)
                                <button class="btn btn-sm btn-primary" type="button"
                                    onclick="openComposeEmailModal()">Compose Email</button>
                            @endif
                        </div>
                            ...
                            @foreach($lead->emails as $email)
                                <div class="timeline-card">
                                    ...
                                    <div class="text-muted small mb-2">From {{ $email->from_email ?: '-' }} | To
                                        {{ $email->to_email ?: '-' }} @if($email->cc)| Cc {{ $email->cc }}@endif</div>
                                    ...
                                    <div class="mt-2 small text-muted">
                                        ...
                                        {{ optional($email->sentBy)->username ?? optional($email->sentBy)->name ?? '-' }}</div>
                                </div>
                            @endforeach
                        @endif
                    </div>
                ...
                <form id="meetingForm">@csrf<div class="modal-header bg-primary text-white">
                    ...
                    <div class="modal-body"><input type="hidden" id="meeting_id">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group"><label>Meeting Type</label><select class="form-control"
                                        id="meeting_type">
    ...
    <script>
        const leadPermissions = { manageNotes: @json($canManageNotes), manageTasks: @json($canManageTasks), createScheduledCalls: @json($canCreateScheduledCalls), logCalls: @json($canLogCalls), editCalls: @json($canEditCalls), completeCalls: @json($canCompleteCalls), scheduleMeetings: @json($canScheduleMeetings), editMeetings: @json($canEditMeetings), completeMeetings: @json($canCompleteMeetings), sendEmails: @json($canSendEmails) }; function formatDateLocal(date) { if (!date) return ''; const d = new Date(date); const offset = d.getTimezoneOffset(); return new Date(d.getTime() - offset * 60000).toISOString().slice(0, 16); } function getRecordCard(src) { if (src && src.jquery) return src.closest('.editable-record'); return $(src).closest('.editable-record'); } function handleReloadResponse(response) { if (response.success) { location.reload(); } else { alert(response.message || 'Something went wrong.'); } } function openNoteModal() { if (!leadPermissions.manageNotes) return; $('#noteModalTitle').text('Add Note'); $('#note_id').val(''); $('#note_text').val(''); $('#noteModal').modal('show'); } function editNote(id, note) { if (!leadPermissions.manageNotes) return; $('#noteModalTitle').text('Edit Note'); $('#note_id').val(id); $('#note_text').val(note || ''); $('#noteModal').modal('show'); } function openTaskModal() { if (!leadPermissions.manageTasks) return; $('#taskModalTitle').text('Add Task'); $('#task_id').val(''); $('#task_subject,#task_description,#task_due_date').val(''); $('#task_status').val('pending'); $('#task_assigned_to').val(''); $('#taskModal').modal('show'); } function editTaskFromCard(src) { if (!leadPermissions.manageTasks) return; const card = getRecordCard(src); $('#taskModalTitle').text('Edit Task'); $('#task_id').val(card.data('id')); $('#task_subject').val(card.data('subject') || ''); $('#task_description').val(card.data('description') || ''); $('#task_assigned_to').val(card.data('assigned') || ''); $('#task_due_date').val(card.data('due') || ''); $('#task_status').val(card.data('status') || 'pending'); $('#taskModal').modal('show'); } function openCallModal(mode) { if (mode === 'scheduled' && !leadPermissions.createScheduledCalls) return; if (mode !== 'scheduled' && !leadPermissions.logCalls) return; $('#call_id').val(''); $('#call_mode').val(mode || 'logged'); $('#callModalTitle').text(mode === 'scheduled' ? 'Create Call' : 'Log Call'); $('#call_type').val('outbound'); $('#call_purpose').val(''); $('#call_notes').val(''); $('#call_duration').val(''); $('#call_status').val(mode === 'scheduled' ? 'scheduled' : 'completed'); $('#call_date').val(formatDateLocal(new Date())); $('#callModal').modal('show'); } function editCallFromCard(src) { if (!leadPermissions.editCalls) return; const card = getRecordCard(src); $('#call_id').val(card.data('id')); $('#call_mode').val(card.data('mode') || 'logged'); $('#callModalTitle').text('Edit Call'); $('#call_type').val(card.data('call-type') || 'outbound'); $('#call_purpose').val(card.data('purpose') || ''); $('#call_notes').val(card.data('notes') || ''); $('#call_duration').val(card.data('duration') || ''); $('#call_status').val(card.data('status') || 'completed'); $('#call_date').val(card.data('date') || ''); $('#callModal').modal('show'); } function openCompleteCallModal(src) { if (!leadPermissions.completeCalls) return; const card = getRecordCard(src); $('#complete_call_id').val(card.data('id')); $('#complete_call_duration').val(card.data('duration') || ''); $('#complete_call_status').val('completed'); $('#complete_call_notes').val(card.data('notes') || ''); $('#completeCallModal').modal('show'); } function openMeetingModal() { if (!leadPermissions.scheduleMeetings) return; $('#meetingModalTitle').text('Schedule Meeting'); $('#meeting_id').val(''); $('#meeting_title,#meeting_description,#meeting_location,#meeting_link,#meeting_duration').val(''); $('#meeting_type').val('virtual'); $('#meeting_status').val('scheduled'); $('#meeting_assigned_to').val(''); $('#meeting_date').val(formatDateLocal(new Date())); $('#meetingModal').modal('show'); } function editMeetingFromCard(src) { if (!leadPermissions.editMeetings) return; const card = getRecordCard(src); $('#meetingModalTitle').text('Edit Meeting'); $('#meeting_id').val(card.data('id')); $('#meeting_title').val(card.data('title') || ''); $('#meeting_description').val(card.data('description') || ''); $('#meeting_type').val(card.data('type') || 'virtual'); $('#meeting_location').val(card.data('location') || ''); $('#meeting_link').val(card.data('link') || ''); $('#meeting_date').val(card.data('date') || ''); $('#meeting_duration').val(card.data('duration') || ''); $('#meeting_assigned_to').val(card.data('assigned') || ''); $('#meeting_status').val(card.data('status') || 'scheduled'); $('#meetingModal').modal('show'); } function openCompleteMeetingModal(id) { if (!leadPermissions.completeMeetings) return; $('#complete_meeting_id').val(id); $('#complete_meeting_notes').val(''); $('#completeMeetingModal').modal('show'); } function openComposeEmailModal() { if (!leadPermissions.sendEmails) return; $('#composeEmailForm')[0].reset(); $('#email_to').val('{{ $lead->email }}'); $('#composeEmailModal').modal('show'); }
        function deleteNote(id) { if (!confirm('Delete this note?')) return; $.post('{{ url("admin/leads/notes") }}/' + id, { _token: '{{ csrf_token() }}', _method: 'DELETE' }, handleReloadResponse); } function deleteTask(id) { if (!confirm('Delete this task?')) return; $.post('{{ url("admin/leads/tasks") }}/' + id, { _token: '{{ csrf_token() }}', _method: 'DELETE' }, handleReloadResponse); } function deleteCall(id) { if (!confirm('Delete this call?')) return; $.post('{{ url("admin/leads/calls") }}/' + id, { _token: '{{ csrf_token() }}', _method: 'DELETE' }, handleReloadResponse); } function deleteMeeting(id) { if (!confirm('Delete this meeting?')) return; $.post('{{ url("admin/leads/meetings") }}/' + id, { _token: '{{ csrf_token() }}', _method: 'DELETE' }, handleReloadResponse); } function markTaskCompleted(id) { $.post('{{ url("admin/tasks") }}/' + id + '/status', { _token: '{{ csrf_token() }}', _method: 'PUT', status: 'completed' }, handleReloadResponse); }
        $(function () {
            $('.editable-record').on('dblclick', function () { const kind = $(this).data('kind'); if (kind === 'note' && leadPermissions.manageNotes) { editNote($(this).data('id'), $(this).data('note')); } else if (kind === 'task' && leadPermissions.manageTasks) { editTaskFromCard($(this)); } else if (kind === 'call' && leadPermissions.editCalls) { editCallFromCard($(this)); } else if (kind === 'meeting' && leadPermissions.editMeetings) { editMeetingFromCard($(this)); } }); $('#noteForm').on('submit', function (e) { e.preventDefault(); const id = $('#note_id').val(); const payload = { _token: '{{ csrf_token() }}', note: $('#note_text').val(), lead_id: '{{ $lead->id }}' }; if (id) { payload._method = 'PUT'; $.post('{{ url("admin/leads/notes") }}/' + id, payload, handleReloadResponse); } else { $.post('{{ route("admin.lead.addNote") }}', payload, handleReloadResponse); } }); $('#taskForm').on('submit', function (e) { e.preventDefault(); const id = $('#task_id').val(); const payload = { _token: '{{ csrf_token() }}', lead_id: '{{ $lead->id }}', subject: $('#task_subject').val(), description: $('#task_description').val(), assigned_to: $('#task_assigned_to').val(), due_date: $('#task_due_date').val(), status: $('#task_status').val() }; if (id) { payload._method = 'PUT'; $.post('{{ url("admin/leads/tasks") }}/' + id, payload, handleReloadResponse); } else { $.post('{{ route("admin.lead.addTask") }}', payload, handleReloadResponse); } });
            $('#callForm').on('submit', function (e) { e.preventDefault(); const id = $('#call_id').val(); const payload = { _token: '{{ csrf_token() }}', lead_id: '{{ $lead->id }}', call_type: $('#call_type').val(), call_purpose: $('#call_purpose').val(), notes: $('#call_notes').val(), duration: $('#call_duration').val(), status: $('#call_status').val(), call_date: $('#call_date').val() }; if (id) { payload._method = 'PUT'; payload.called_by = '{{ auth()->id() }}'; $.post('{{ url("admin/leads/calls") }}/' + id + '/full', payload, handleReloadResponse); } else if ($('#call_mode').val() === 'scheduled') { payload.status = 'scheduled'; $.post('{{ route("admin.lead.scheduleCall") }}', payload, handleReloadResponse); } else { $.post('{{ route("admin.lead.addCall") }}', payload, handleReloadResponse); } }); $('#completeCallForm').on('submit', function (e) { e.preventDefault(); $.post('{{ url("admin/leads/calls") }}/' + $('#complete_call_id').val(), { _token: '{{ csrf_token() }}', _method: 'PUT', duration: $('#complete_call_duration').val(), status: $('#complete_call_status').val(), notes: $('#complete_call_notes').val() }, handleReloadResponse); }); $('#meetingForm').on('submit', function (e) { e.preventDefault(); const id = $('#meeting_id').val(); const payload = { _token: '{{ csrf_token() }}', lead_id: '{{ $lead->id }}', title: $('#meeting_title').val(), description: $('#meeting_description').val(), meeting_type: $('#meeting_type').val(), location: $('#meeting_location').val(), meeting_link: $('#meeting_link').val(), meeting_date: $('#meeting_date').val(), duration: $('#meeting_duration').val(), assigned_to: $('#meeting_assigned_to').val(), status: $('#meeting_status').val() }; if (id) { payload._method = 'PUT'; $.post('{{ url("admin/leads/meetings") }}/' + id, payload, handleReloadResponse); } else { delete payload.status; $.post('{{ route("admin.lead.scheduleMeeting") }}', payload, handleReloadResponse); } }); $('#completeMeetingForm').on('submit', function (e) { e.preventDefault(); $.post('{{ url("admin/leads/meetings") }}/' + $('#complete_meeting_id').val() + '/complete', { _token: '{{ csrf_token() }}', notes: $('#complete_meeting_notes').val() }, handleReloadResponse); }); $('#email_template').on('change', function () { const o = $(this).find(':selected'); $('#email_subject').val(o.data('subject') || ''); $('#email_body').val(o.data('body') || ''); }); $('#composeEmailForm').on('submit', function (e) { e.preventDefault(); $.post('{{ route("admin.lead.sendEmail") }}', $(this).serialize(), handleReloadResponse); });
        });
    </script>
@endsection
