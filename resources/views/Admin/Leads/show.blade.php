                                ...
                                @can('leads_notes')
                                    <div class="tab-pane fade {{ $defaultTab === 'notes-pane' ? 'show active' : '' }}" id="notes-pane">
                                        ...
                                        @forelse($lead->notes as $note)
                                            <div class="timeline-card editable-record" data-kind="note" data-id="{{ $note->id }}" data-note="{{ e($note->note) }}">
                                                <div class="d-flex justify-content-between align-items-start">
                                                    ...
                                                    <div class="text-right">
                                                        <small class="text-muted d-block">{{ optional($note->created_at)->format('d M Y, h:i A') }}</small>
                                        ...
                                        <div class="col-lg-6 mb-4">
                                            ...
                                            @forelse($scheduledCalls as $call)
                                                <div class="timeline-card editable-record" data-kind="call" data-mode="scheduled"
                                                    ...
                                                    data-duration="{{ e($call->duration) }}" data-status="{{ $call->status }}" data-call-owner="{{ $call->call_owner ?: $call->called_by }}"
                                                ...
                                                <div class="timeline-card editable-record" data-kind="call" data-mode="logged"
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-start">
                                                        <div>
                                                            ...
                                                            <div class="text-muted small mb-2">{{ ucfirst($call->call_type) }} call | Owner {{ optional($call->callOwner)->username ?? optional($call->callOwner)->name ?? optional($call->calledBy)->username ?? optional($call->calledBy)->name ?? '-' }}</div>
                                                            <p class="mb-1">{{ $call->notes ?: 'No notes' }}</p><small
                        ...
                        <div class="modal-content">
                            <form id="callForm">@csrf<div class="modal-header bg-primary text-white">
                                ...
                                <div class="modal-body"><input type="hidden" id="call_id"><input type="hidden" id="call_mode"
                                    ...
                                    <div class="form-group"><label>Call Owner</label><select class="form-control" id="call_owner" required><option value="">Select User</option>@foreach($users as $user)<option value="{{ $user->id }}">{{ $user->username ?? $user->name }}</option>@endforeach</select></div>
