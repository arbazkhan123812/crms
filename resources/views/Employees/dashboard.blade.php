                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <span class="text-muted small">{{ round(($presentCount / max($presentCount + $absentCount, 1)) * 100) }}% attendance</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <span class="text-muted small">{{ $lateCount > 0 ? 'Needs improvement' : 'Perfect punctuality' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    @if($todayAttendance)
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-4">
                            <div>
                                <span class="badge badge-{{ $todayAttendance->status == 'present' ? 'success' : 'warning' }} px-3 py-1">
                                    <i class="fas fa-{{ $todayAttendance->status == 'present' ? 'check-circle' : 'exclamation-circle' }} mr-1"></i>
                                    {{ ucfirst($todayAttendance->status) }}
                                </span>
                                @if($todayAttendance->is_late)
                                    <span class="badge badge-danger ml-2 px-3 py-1">
                                        <i class="fas fa-exclamation-triangle mr-1"></i>Late by {{ $todayAttendance->late_minutes }} min
                                    </span>
                                @endif
                            </div>
                        ...
                        @if($todayAttendance->check_in && $todayAttendance->check_out)
                            <div class="alert alert-success bg-light border-0 text-center py-2 mb-0">
                                ...
                                <span>You have successfully completed your day. Checked out at <strong>{{ Carbon\Carbon::parse($todayAttendance->check_out)->format('h:i A') }}</strong></span>
                            </div>
                        @endif
                    @else
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="border rounded p-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <span class="badge bg-primary text-white">{{ $balance->remaining_days }}/{{ $balance->total_days }}</span>
                                    </div>
                                    <div class="progress" style="height: 6px;">
                                        <div class="progress-bar bg-success" style="width: {{ $balance->utilization_percentage }}%"></div>
                                    </div>
                                    <small class="text-muted d-block mt-1">{{ $balance->utilization_percentage }}% utilized</small>
                                </div>
                            </div>
                        @empty
